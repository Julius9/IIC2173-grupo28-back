...
// Escribe la inicialización en la consola
...
async function calculateRecommendations(userCoords, flights) {
  console.log('Calculating recommendations for userCoords:', userCoords, 'flights:', flights.map(f => f.id));
  const recommendations = [];
  for (const flight of flights) {
    const destinationCoords = await getAirportCoordinates([flight.arrival_airport_id]);
    if (destinationCoords[0].lat && destinationCoords[0].lon) {
